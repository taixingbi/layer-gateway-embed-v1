...
**Correlation:** **`GET /health`** and **`GET /v1/models`** do not require correlation headers. For **`POST /v1/embeddings`**, both **`X-Request-Id`** and **`X-Session-Id`** must be present and non-empty (after trim); otherwise the gateway returns **400** with `missing` listing which headers were absent. For other routes, if **`X-Request-Id`** is omitted it remains `-` in logs/forwarded headers; **`X-Session-Id`** is optional. Responses echo **`X-Request-Id`**; **`X-Session-Id`** is echoed when sent. For `/v1/*` proxy routes, these headers are forwarded to the vLLM backend. Logs are **JSON lines** to stderr (and optional Loki) with fields **`ts`** (America/New_York), **`request_id`**, **`session_id`**, **`method`**, **`path`**, **`status`** (HTTP status once the response has started; **`-`** until then, except **400** for rejected POST `/v1/embeddings`), **`message`**. Outside any HTTP request (startup, Loki init), **`method`**, **`path`**, **`status`**, **`request_id`**, and **`session_id`** are **`-`**.
...
**Observability:** Successful proxy logs use message **`proxied upstream response`** with JSON fields **`status`** (upstream HTTP status), **`duration_ms`**, **`backend`** (chosen vLLM base URL), and optionally **`gpu`** when `ROUTER_GPU_TRACE_HEADER` is set (upstream header value, or `-` if missing). **`status`** in JSON may come from that `extra` block before ASGI sends the response; after `http.response.start`, middleware can also populate **`status`** from context for other log lines.
